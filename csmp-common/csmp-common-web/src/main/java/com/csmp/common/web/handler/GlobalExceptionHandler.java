...
public class GlobalExceptionHandler {
    ...
    private <T> ResponseEntity<R<T>> respond(R<T> body) {
        return ResponseEntity.status(HttpStatusCode.valueOf(body.getCode())).body(body);
    }
    ...
    public ResponseEntity<R<Void>> handleHttpRequestMethodNotSupported(HttpRequestMethodNotSupportedException e,
        ...
        return respond(R.fail(HttpStatus.HTTP_BAD_METHOD, e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleServiceException(ServiceException e, HttpServletRequest request) {
        ...
        return respond(ObjectUtil.isNotNull(code) ? R.fail(code, e.getMessage()) : R.fail(e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleServletException(ServletException e, HttpServletRequest request) {
        ...
        return respond(R.fail(e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleBaseException(BaseException e, HttpServletRequest request) {
        log.error(e.getMessage());
        return respond(R.fail(e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleMissingPathVariableException(MissingPathVariableException e, HttpServletRequest request) {
        ...
        return respond(R.fail(String.format("请求路径中缺少必需的路径变量[%s]", e.getVariableName())));
    }
    ...
    public ResponseEntity<R<Void>> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.error("请求参数类型不匹配'{}',发生系统异常.", requestURI);
        return respond(R.fail(String.format("请求参数类型不匹配，参数[%s]要求类型为：'%s'，但输入值为：'%s'", e.getName(), e.getRequiredType().getName(), e.getValue())));
    }
    ...
    public ResponseEntity<R<Void>> handleNoHandlerFoundException(NoHandlerFoundException e, HttpServletRequest request) {
        ...
        return respond(R.fail(HttpStatus.HTTP_NOT_FOUND, e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleJsonParseException(JsonParseException e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.error("请求地址'{}' 发生 JSON 解析异常: {}", requestURI, e.getMessage());
        return respond(R.fail(HttpStatus.HTTP_BAD_REQUEST, "请求数据格式错误（JSON 解析失败）：" + e.getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleHttpMessageNotReadableException(HttpMessageNotReadableException e, HttpServletRequest request) {
        ...
        return respond(R.fail(HttpStatus.HTTP_BAD_REQUEST, "请求参数格式错误：" + e.getMostSpecificCause().getMessage()));
    }
    ...
    public ResponseEntity<R<Void>> handleSpelException(ExpressionException e, HttpServletRequest request) {
        log.error("请求地址'{}'，SpEL解析异常: {}", request.getRequestURI(), e.getMessage());
        return respond(R.fail(HttpStatus.HTTP_INTERNAL_ERROR, "SpEL解析失败：" + e.getMessage()));
    }
